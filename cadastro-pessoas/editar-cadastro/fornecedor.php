        ...
        <main class="content">
            ...
            <?php
                ...
                if ($tipo_usuario === "fornecedor") {
                    ...
                    $query = getQuery('select_all_fornecedores_by_id');

                    // Preparar a consulta para fornecedores
                    $stmt = $conn->prepare($query);
                    $stmt->bind_param("i", $id); // Vincula o ID
                    $stmt->execute();
                    $stmt->bind_result($id, $nome, $email, $telefone, $data_nascimento, $pais, $cep, $estado, $cidade, $bairro, $rua, $numero, $complemento, $data_cadastro, $nome_empresa, $documento); // 15 variáveis
                    ...
                    if ($stmt->fetch()) {
                        ...
                        echo '<label for="nome_empresa">Nome da Empresa:</label>';
                        echo '<input type="text" id="nome_empresa" name="nome_empresa" value="' . htmlspecialchars($nome_empresa) . '" required><br>';
                
                        echo '<label for="documento">Documento:</label>';
                        echo '<input type="text" id="documento" name="documento" value="' . htmlspecialchars($documento) . '" required><br>';
