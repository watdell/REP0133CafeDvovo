        ...
        <main class="content">
            ...
            <?php
                ...
                if ($tipo_usuario === "funcionario") {
                    ...
                    $query = getQuery('select_all_funcionarios_by_id');
                    // Preparar a consulta para funcionários
                    $stmt = $conn->prepare($query);
                    $stmt->bind_param("i", $id); // Vincula o ID
                    $stmt->execute();
                    $stmt->bind_result($id, $nome, $email, $telefone, $data_nascimento, $pais, $cep, $estado, $cidade, $bairro, $rua, $numero, $complemento, $matricula, $cargo, $salario, $data_cadastro);
                    ...
                    if ($stmt->fetch()) {
                        ...
                        echo '<input type="text" id="matricula" name="matricula" value="' . htmlspecialchars($matricula) . '" required><br>';
                
                        echo '<label for="cargo">Cargo:</label>';
                        echo '<input type="text" id="cargo" name="cargo" value="' . htmlspecialchars($cargo) . '" required><br>';
                
                        echo '<label for="salario">Salário:</label>';
                        echo '<input type="text" id="salario" name="salario" value="' . htmlspecialchars($salario) . '" required><br>';
